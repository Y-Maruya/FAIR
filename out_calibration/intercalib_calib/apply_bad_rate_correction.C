...
std::map<int, double> calculateBadRates(const std::vector<RunData> &allRuns) {
  ...
  // First pass: compute RMS/SIGMA threshold for each run
  ...
  for (size_t run_idx = 0; run_idx < allRuns.size(); ++run_idx) {
    ...
    double threshold = mean_ratio + rms_ratio * 10.0;
  ...
  for (int cid : all_cellids) {
    ...
    for (size_t run_idx = 0; run_idx < allRuns.size(); ++run_idx) {
      ...
      if (it != run.byCellId.end()) {
        ...
        // Start with bit 0 and 1 from quality_flag
        uint32_t effective_flags = entry.quality_flag & (HGLG_LOW_STAT | HGLG_FIT_FAILED);
        
        // Recompute bit 2 (BAD_RMSPERSIGMA) from distance metrics
        double threshold = run_thresholds[run.runNumber];
        if (entry.distance_sigma > 0 && entry.distance_rms >= 0) {
          double ratio = entry.distance_rms / entry.distance_sigma;
          if (ratio > threshold) {
        ...
        // Count as bad if any of bits 0, 1, or 2 are set
        if ((effective_flags & (HGLG_LOW_STAT | HGLG_FIT_FAILED | HGLG_BAD_RMSPERSIGMA)) != 0) {
          bad_count++;
        }
      }
    }
  ...
  for (int layer = 0; layer < 41; ++layer) {
    ...
    for (int chip = 0; chip < 9; ++chip) {
      for (int channel = 0; channel < 36; ++channel) {
        ...
        // Collect thresholds for BAD_RMSPERSIGMA calculation
        std::map<int, double> run_thresholds;
        for (size_t run_idx = 0; run_idx < allRuns.size(); ++run_idx) {
          const auto &run = allRuns[run_idx];
          std::vector<double> run_ratios;
          for (const auto &entry : run.entries) {
            if (entry.distance_sigma > 0 && entry.distance_rms >= 0) {
              run_ratios.push_back(entry.distance_rms / entry.distance_sigma);
            }
          }
          double mean_ratio = run_ratios.empty() ? 0.0 : std::accumulate(run_ratios.begin(), run_ratios.end(), 0.0) / run_ratios.size();
          double rms_ratio = 0.0;
          if (!run_ratios.empty()) {
            double sum_sq_diff = 0.0;
            for (double ratio : run_ratios) sum_sq_diff += (ratio - mean_ratio) * (ratio - mean_ratio);
            rms_ratio = std::sqrt(sum_sq_diff / run_ratios.size());
          }
          run_thresholds[run.runNumber] = mean_ratio + rms_ratio * 10.0;
        }

        const uint32_t HGLG_LOW_STAT = 1 << 0;
        const uint32_t HGLG_FIT_FAILED = 1 << 1;
        const uint32_t HGLG_BAD_RMSPERSIGMA = 1 << 2;
        ...
        for (const auto &run : allRuns) {
          for (const auto &entry : run.entries) {
            if (entry.layer == layer && entry.chip == chip && entry.channel == channel) {
              ...
              // Compute effective quality flag with recalculated BAD_RMSPERSIGMA
              uint32_t effective_flags = entry.quality_flag & (HGLG_LOW_STAT | HGLG_FIT_FAILED);
              double threshold = run_thresholds[run.runNumber];
              if (entry.distance_sigma > 0 && entry.distance_rms >= 0) {
                double ratio = entry.distance_rms / entry.distance_sigma;
    ...
    for (const auto &run : allRuns) {
      ...
      for (int same_run : same_data_runs) {
        ...
        j["PerChannel"]["HG_SaturationPoint"] = hg_adc_saturation_arr;
...
void apply_bad_rate_correction(const char *baseDir = ".",
  ...
  if (!allRuns.empty()) {
    ...
    for (size_t i = 0; i < std::min(size_t(5), allRuns[0].entries.size()); ++i) {
      ...
      std::cout << "  Entry " << i << ": cellid=" << e.cellid << " layer=" << e.layer 
                ...
                << " is_bad=" << ((e.quality_flag & 0x7) != 0 ? "yes" : "no") << std::endl;
    }
  }

  // Calculate bad rate information
  std::cout << "\nCalculating bad rates..." << std::endl;
  auto badRates = calculateBadRates(allRuns);
  ...
  std::cout << "  Masked channels are " << std::endl;
  for (const auto &[cid, rate] : badRates) {
    if (rate >= 50.0) {
      std::cout << "    CellID " << cid << ": " << std::fixed << std::setprecision(1) << rate << "% bad rate\n";
    }
  }
  ...
  applyCorrectionAndWriteJson(allRuns, badRates, outDir);
