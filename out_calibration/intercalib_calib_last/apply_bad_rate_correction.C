    ...
    for (int chip = 0; chip < 9; ++chip) {
      for (int channel = 0; channel < 36; ++channel) {
        ...
        if (should_mask) {
          ...
          double p0_RMS = 0.0;
          double p1_RMS = 0.0;
          p0_RMS = std::sqrt(std::accumulate(p0_vals.begin(), p0_vals.end(), 0.0, [p0_arr, idx](double sum, double val) {
            return sum + (val - p0_arr[idx]) * (val - p0_arr[idx]);
          }) / p0_vals.size());
          p1_arr[idx] = std::accumulate(p1_vals.begin(), p1_vals.end(), 0.0) / p1_vals.size();
          p1_RMS = std::sqrt(std::accumulate(p1_vals.begin(), p1_vals.end(), 0.0, [p1_arr, idx](double sum, double val) {
            return sum + (val - p1_arr[idx]) * (val - p1_arr[idx]);
          }) / p1_vals.size());
          if (!p0_err_vals.empty()) {
            p0_err_arr[idx] = std::hypot(std::accumulate(p0_err_vals.begin(), p0_err_vals.end(), 0.0) / p0_err_vals.size(), p0_RMS);
          }
          if (!p1_err_vals.empty()) {
            p1_err_arr[idx] = std::hypot(std::accumulate(p1_err_vals.begin(), p1_err_vals.end(), 0.0) / p1_err_vals.size(), p1_RMS);
          }
