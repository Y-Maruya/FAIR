...
void drawNPointsSlopeCorrelations(const std::vector<RunData> &runs,
  ...
  for (size_t rank = 0; rank < nCells; ++rank) {
    ...
    for (const auto &run : runs) {
      ...
      slopeErrors.push_back((std::isfinite(it->second.slope_error) &&
                             it->second.slope_error >= 0.0)
                                ? it->second.slope_error
                                : 0.0);
    ...
    for (size_t i = 0; i < slopes.size(); ++i) {
      yMin = std::min(yMin, slopes[i] - slopeErrors[i]);
      yMax = std::max(yMax, slopes[i] + slopeErrors[i]);
    }
    ...
    TCanvas *c = new TCanvas(Form("c_slope_vs_npoints_cell%d", info.cellid),
                             ...
                             1100, 850);
    std::vector<double> xErrors(nPoints.size(), 0.0);
    TGraphErrors *g = new TGraphErrors(nPoints.size(), nPoints.data(), slopes.data(),
                                       xErrors.data(), slopeErrors.data());
    ...
    for (size_t i = 0; i < nPoints.size(); ++i) {
      tl.DrawLatex(nPoints[i] + labelDx,
                   slopes[i] + slopeErrors[i] + labelDy,
                   labels[i].c_str());
    }
