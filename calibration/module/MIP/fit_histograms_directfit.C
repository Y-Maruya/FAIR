...
// Sum the Landau-Gauss expectation in histogram-bin units.
// Use the same convention everywhere below: TF1::Eval(bin center) is treated
// as the expected bin content, because this is the convention used by TH1::Fit.
double sumLandauGausEvalInRange(TF1* fLandauGaus, TH1D* h, double xmin, double xmax) {
    ...
    for (int ibin = 1; ibin <= h->GetNbinsX(); ++ibin) {
        ...
        if (x <= xmin) continue;
        if (x > xmax) continue;
        sum += fLandauGaus->Integral(x - 0.5 * h->GetBinWidth(ibin), x + 0.5 * h->GetBinWidth(ibin))/h->GetBinWidth(1);  // Get fit value for this bin by integrating over the bin width and normalizing by bin width
    }
    return sum;
}
...
TF1* makeDirectThresholdModel(
    ...
    auto model = [fLandauGaus, lg_scale](double* xx, double* par) {
        const double adc = xx[0];
        const double threshold = par[0];
        const double width = par[1];
        return lg_scale * fLandauGaus->Eval(adc) * thresholdEfficiencyErf(adc, threshold, width);
    };
...
TF1* makeScaledLandauGausModel(
    ...
    auto model = [fLandauGaus, lg_scale](double* xx, double*) {
        return lg_scale * fLandauGaus->Eval(xx[0]);
    };
...
double computeDirectThresholdEfficiencyEval(
    TF1* fLandauGaus,
    TH1D* h,
    double lg_scale,
    double threshold,
    ...
    for (int ibin = 1; ibin <= h->GetNbinsX(); ++ibin) {
        ...
        const double fit = fLandauGaus->Integral(x - 0.5 * h->GetBinWidth(ibin), x + 0.5 * h->GetBinWidth(ibin))/h->GetBinWidth(1) * lg_scale;  // Get fit value for this bin by integrating over the bin width and normalizing by bin width
...
void fit_histograms_A(const char* input_file = "mip.root", const char* output_file = "mip_fitted.root") {
    ...
    std::unordered_map<int, double> totalAreaMap = createTotalAreaMap("/afs/cern.ch/user/y/ymaruya/private/FASERlink/AHCAL/FAIR/out/SimMuon/mip_neighborcheck_nofit.root");
    fout->cd();
...
3220002
        ...
        std::cout << "Drawing histogram " << histName << " with fit class: " << className << std::endl;
        cFit->Update();
        cFit->SaveAs(Form("/afs/cern.ch/user/y/ymaruya/private/FASERlink/AHCAL/FAIR/out_calibration/mip_calib/21987-22163/test/fit_example_cellid_%d.png", res.cellid));
        ...
        // std::cout << "Histogram " << histName << " has small gaus_sigma = " << fitOut.gaus_sigma << ", retrying fit with min_gaus_sigma = 20" << std::endl;
            ...
            if (fitOut.fLandauGaus) {
                ...
                double N_over50 = sumDataInRange(h, 50.0, h->GetXaxis()->GetXmax());
                ...
                double A_actual = sumLandauGausEvalInRange(
                ...
                if (A_def <= 0.0 || A_actual <= 0.0) {
                    ...
                    const double lg_scale = 1;

                    TF1* f_direct = makeDirectThresholdModel(
                        fitOut.fLandauGaus,
                        lg_scale,
                        x_direct_min,
                        x_direct_max,
                    ...
                    TF1* f_scaled_lg = makeScaledLandauGausModel(
                        fitOut.fLandauGaus,
                        lg_scale,
                        x_direct_min,
                        x_direct_max,
                    ...
                    if (!f_direct || !f_scaled_lg) {
                        ...
                        const double eff_threshold = computeDirectThresholdEfficiencyEval(
                            fitOut.fLandauGaus,
                            h,
                            lg_scale,
                            threshold_out,
                        ...
                        for (int ibin = 1; ibin <= h->GetNbinsX(); ++ibin) {
                            ...
                            const double data = h->GetBinContent(ibin);
                            const double fit = lg_scale * fitOut.fLandauGaus->Integral(x - h->GetBinWidth(ibin)/2, x + h->GetBinWidth(ibin)/2)/h->GetBinWidth(ibin);
                        ...
                        std::cout << "A_actual_sum_eval     = " << A_actual << std::endl;
                        std::cout << "LG scale              = " << lg_scale << std::endl;
                        std::cout << "N_all                 = " << N_all << std::endl;
                        std::cout << "N_over50              = " << N_over50 << std::endl;
                        ...
                        latexDirect.DrawLatex(0.55, 0.65, Form("Eff data/fit range = %.1f %%", eff_data_actual*100.0));
                        latexDirect.DrawLatex(0.55, 0.60, Form("Scale = %.3f", lg_scale));
                        latexDirect.DrawLatex(0.55, 0.55, Form("Chi2/NDF = %.1f/%d", f_direct->GetChisquare(), f_direct->GetNDF()));
                    }
                }                
            }
            cRetry->Update();
            cRetry->SaveAs(Form("/afs/cern.ch/user/y/ymaruya/private/FASERlink/AHCAL/FAIR/out_calibration/mip_calib/21987-22163/test/fit_retry_cellid_%d.png", res.cellid));
            delete cRetry;
            cRetry = nullptr;
            if (h_ratio) {
                delete h_ratio;
...
void fit_histograms_directfit() {
    fit_histograms_A("/afs/cern.ch/user/y/ymaruya/private/FASERlink/AHCAL/FAIR/out_calibration/mip_calib/21987-22163/mip_neighborcheck_nofit.root", "/afs/cern.ch/user/y/ymaruya/private/FASERlink/AHCAL/FAIR/out_calibration/mip_calib/21987-22163/mip_neighborcheck_fitted_tmps.root");
}
...
int main(int argc, char** argv) {
    if (argc == 1) {
        fit_histograms_directfit();  // Default mode with plotting
    }
    if (argc == 3) {
