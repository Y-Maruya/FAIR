...
void compare_Input_Pattern()
{
	...
	const char* filePattern = "../out/run%d/input_eff.root";
	...
	const std::vector<Item> items = {
        ...
        // {21739, "300 Hz", ""},
        // {21729, "500 Hz", ""},
		{21915, "Delay 10*6.25 ns", ""},
		{21918, "Delay 5*6.25 ns", ""},
		{21897, "Delay 0*6.25 ns", ""}
	};
	...
	const char* outPng = "compare_Input_Pattern.png";
	const bool normalizeToUnitArea = true; // set to true if you want to compare shapes regardless of total counts
	...
	// --- Basic checks ---
	if (items.empty()) {
		::Error("compare_Input_Pattern", "items is empty. Please add at least one Item.");
		return;
	}

	gStyle->SetOptStat(0);

	TCanvas* c = new TCanvas("c_compare_Input_Pattern", "compare: h_loss", 1000, 900);
	...
	for (size_t i = 0; i < items.size(); ++i) {
		...
		if (!f || f->IsZombie()) {
			::Warning("compare_Input_Pattern", "Cannot open: %s (skip)", fpath.c_str());
			if (f) f->Close();
			continue;
		}
		...
		if (!h) {
			::Warning("compare_Input_Pattern", "Histogram not found: %s in %s (skip)", hpath.c_str(), fpath.c_str());
			f->Close();
			continue;
		}
	...
	if (hists.empty()) {
		::Error("compare_Input_Pattern", "No histograms loaded. Check filePattern/items and histogram paths.");
		return;
	}
	...
	if (refLoadedIndex < 0) {
		::Error("compare_Input_Pattern", "Reference item index %d not loaded. Check items and files.", referenceItemIndex);
		return;
	}
	...
	for (size_t i = 0; i < hists.size(); ++i) {
		TH1* h = hists[i];
		h->SetTitle(";Layer;Input Pattern");
