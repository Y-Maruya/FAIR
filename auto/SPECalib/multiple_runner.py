...
parser = argparse.ArgumentParser(description="Run spe-calibration for a range of runs.")
parser.add_argument("--start_run_number", "-s", type=int, required=True,
                    help="The starting run number for spe-calibration.")
parser.add_argument("--end_run_number", "-e", type=int, default=None,
                    help="The ending run number for spe-calibration inclusive.")

args = parser.parse_args()
start_run_number = args.start_run_number
end_run_number = args.end_run_number

workdir = "/afs/cern.ch/user/y/ymaruya/private/FASERlink/AHCAL/FAIR/auto/SPECalib"
...
while True:
    ...
    result = subprocess.run(
        ["bash", "run_specalib.sh", str(start_run_number)],
        cwd=workdir,
