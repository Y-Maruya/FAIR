...
public:
    ...
    */
    ...
    void write_event_counter(long long event_counter) {
        ActiveTreeGuard active_tree_guard(m_active_tree, ActiveTree::Event);
    ...
    void write_run_context(const RunContext& run_context, long long start_event, long long end_event) {
        ActiveTreeGuard active_tree_guard(m_active_tree, ActiveTree::RunContext);
    ...
    private:
        ...
        class ActiveTreeGuard {
        public:
            ActiveTreeGuard(ActiveTree& slot, ActiveTree next)
                : m_slot(slot), m_prev(slot) {
                m_slot = next;
            }
            ~ActiveTreeGuard() { m_slot = m_prev; }

            ActiveTreeGuard(const ActiveTreeGuard&) = delete;
            ActiveTreeGuard& operator=(const ActiveTreeGuard&) = delete;
        private:
            ActiveTree& m_slot;
            ActiveTree m_prev;
        };
